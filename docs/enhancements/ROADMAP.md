...
| C9 | **Per-sport stdev recalibration pass (vs game results)** | Low | Small | Follow-up logged 2026-06-29 during the spread de-vig investigation, then broadened by a cross-sport check against ESPN game results. **The de-vig fix (PR #215) is global — no per-sport spread/total *bug* remains.** Remaining items are stdev calibration only, and the *dangerous* direction (too-low stdev → over-confident → inflated edges, the soccer pattern) **does not replicate outside soccer.** Findings: **(a) soccer TOTAL `1.5` too LOW** — realized WC total-goals stdev **1.86** (n=74); over-confident totals; left as-is because soccer totals are profitable (75% WR/+24% ROI, n=24). **(b) NBA TOTAL `20.7` slightly too WIDE** vs realized 19.78 (n=55) — the R2 `×1.15` bump overshot; *safe* direction (under-confident, costs edges, never fakes them); NBA totals profitable so leave. **(c) NCAAMB TOTAL `17.6` too WIDE** vs realized 14.46 (n=20, thin) — safe direction. **(d) NCAAMB MARGIN `12.1` possibly LOW** (realized uncond 15.62, n=20 thin; C8 once hinted ~14.7) — the only candidate in the risky direction, but weak/unconfirmed and offset by (c); C8 auto-recalibration should catch it once enough NCAAMB spreads settle. **Well-calibrated (no action):** NHL (margin 2.5≈2.43, total 2.2≈2.06), NFL (13.5/13.0 ≈ 13.94/13.14), MLB (4.03/3.45 vs Poisson 3.09–uncond), NBA margin (13.8 < uncond 15.46), NCAAF (15.0/14.0 < uncond, fine). **Stance:** no value changes — recalibrate only with a deliberate backtest, and weight the conditional (residual) stdev, since realized unconditional figures are upper bounds. Pair with C6/C8. Code carries pointer comments at the `SPORT_MARGIN_STDEV["soccer"]` and `SPORT_TOTAL_STDEV["soccer"]` lines. |
