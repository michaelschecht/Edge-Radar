...
| L1 | **Live in-play odds (real-time edges on in-progress games)** | High | Medium (phased) | Design doc: [`live-in-play-odds-design.md`](live-in-play-odds-design.md). Root cause of F44 phantom live edges is the **no-TTL in-process odds cache** (+ 300s file cache), *not* missing data — Odds API `/odds` already returns in-play odds (`commence_time < now`). **Phase 1** = freshness fix: TTL the in-process cache + a short live TTL for in-progress games. Phase 2 = per-event `/events/{id}/odds` refresh. Phase 3 (real-time polling) likely out of scope. Caveats: quota (`markets×regions`), uneven US in-play book coverage (soccer good, MLB spotty/suspended), execution lag. Possible opt-in gate `ALLOW_LIVE_BETS`. Scoped 2026-06-20; deferred by user. |
