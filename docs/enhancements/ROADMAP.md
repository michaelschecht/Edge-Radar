...
| C8-followup | **Fix C8 calibration statistics** | High | Small | **2026-06-23 code review.** C8's fail-safe layer is now hardened (value validation, atomic write, version fallback — see CHANGELOG), but the *math* is unsound: per-sport stdev multipliers are computed from `n≥5` settled bets with no significance gate, and the gap measured is `mean fair_value of gate-selected bets` vs their win rate — which conflates selection bias / vig / line movement with CDF-stdev miscalibration. Result: the live cache widened NBA totals +39% and clamped soccer margin to the −20% floor on tiny samples, applied to live pricing. Fix: raise the sample floor to ~30, add a CI/significance check before moving a sport, and reconsider calibrating against per-game outcomes rather than selected-bet win rates. **Interim:** deleting `data/cache/calibration_stdevs.json` reverts to the hardcoded R2 stdevs with zero code change. |
