...
|----|------|--------|--------|-------|
| ~~PM0~~ | **Spike follow-through** | — | Small | **DONE 2026-07-14.** Restored `.claude/skills/polymarket/` API refs (to scratchpad); smoke-tested Gamma live → HTTP 200, not geoblocked, all fields present. **Key finding that redirected Phase 1:** Polymarket's sports markets are **futures/props/politics**, not per-game lines (0 MLB game markets; top markets are World Cup Winner $4.2B, F1 champion, retirement props). So Phase 1 pivoted from "sports game-lines" to **championship futures** (reuses the existing `futures_edge` outright model). |
...
| PM2 | **Phase 2 — execution (real funds)** | High | Large | **Plumbing half SHIPPED 2026-07-20 (PM2a):** `MarketClient` Protocol (7-method contract: balance/positions/create/orders/cancel/fills/settlements) canonical in `scripts/shared/market_client.py` (re-exported via `app/domain/`), `KalshiClient` verified conformant (class-level + signature-coverage tests), `get_market_client(venue)` factory, and `--venue` plumbing on the executor (`run`/`status`; `polymarket` refuses cleanly until the write half exists). +21 tests (578 total). **Remaining (the real-money half, gated on the dry-run edge window + operator answers):** `PolymarketClient` write half via `py-clob-client` (EIP-712 order signing, USDC allowances, fills/cancel). **Wallet private key = direct fund access** — new secrets handling, detect-secrets baseline, dedicated low-balance trading wallet. Test with $1–2 stakes first. Risk gates reused as-is. |
