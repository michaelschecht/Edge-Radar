...
| PM1b | **Event discovery for the other futures (NFL/MLB/NBA/NHL)** | Medium | Small | The `PM_FUTURES` config prices any future given its Gamma event; World Cup works via its confirmed slug, but the keyword-search fallback didn't locate the Super Bowl / World Series / NBA / Stanley Cup events (not in the first 300 active events, or titled differently). Nail each event's exact Gamma **slug** or use **tag_id** filtering so the full config resolves. Small, unblocks year-round futures coverage. |
