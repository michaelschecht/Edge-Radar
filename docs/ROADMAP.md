...
|----|------|--------|--------|-------|
| **PM2c-0** | 🔴 **MOST URGENT — resolve the wallet identity mismatch (operator action, blocks everything downstream)** | High | Small | **Where it stands (2026-07-20):** the PM2b auth smoke test **passed mechanically** — the exported key is valid, EIP-712 signing + CLOB L2 credential derivation succeeded (`sig_type=1`; signer EOA `0xB4C88a…1332` ≠ funder, as expected for a proxy account) — **but the account is empty**: funder `0x3D295D…6591` shows $0 via three independent reads (CLOB collateral, on-chain USDC/USDC.e on Polygon, Data-API portfolio value). **Diagnosis:** operator sees the funds in the *phone app* but the *desktop* session (same email) shows $0 → almost certainly **two distinct Magic accounts** — Polymarket creates a separate wallet per sign-in *method* (Google vs Apple vs typed-email magic link), so the desktop-exported key + address belong to an empty twin. Username `toastyllama6297` has no public web profile (404) — couldn't resolve the funded address that way. **Operator's fix (step-by-step):** (1) note which button the phone app signs in with (Google / Apple / email-code — check app Settings if unsure); (2) log out on desktop, log back in with that *exact same button*; balance appearing on desktop = right account; (3) copy the profile **Address** and re-export the key (`polymarket.com/settings?tab=export-private-key` or `reveal.magic.link/polymarket` with the same sign-in button); (4) replace **both** `POLYMARKET_PRIVATE_KEY` and `POLYMARKET_FUNDER_ADDRESS` in `.env`; (5) have Claude re-run the read-only verification (balance + positions probe — it also derives the signer address and cross-checks the funder on-chain, so a mismatch is caught immediately). Watch-out: if the phone uses **Apple sign-in**, private-relay email can silently break the "same email" assumption — the Magic reveal flow must use the Apple path. Alternative address confirmations: the app's **Share profile** link (embeds `polymarket.com/profile/0x…`) or the **Deposit** screen's USDC receive address. NOTE: `.env` currently has `DRY_RUN=false` (normal for live Kalshi) — fine today since no Polymarket order path is wired, but PM2c goes live the moment it ships. |
