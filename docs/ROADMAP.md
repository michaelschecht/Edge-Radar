...
|----|------|--------|--------|-------|
| **PM2c-0** | 🔴 **MOST URGENT — rebuild execution on the Polymarket US retail API (was "wallet identity mismatch")** | High | Med | **CORRECTED 2026-07-20 — root cause found; supersedes the empty-twin theory below.** Polymarket support confirmed the operator's funded account is the **CFTC-regulated "Polymarket US"** product, accessible **only via the iOS app** (the website is a separate, unavailable product). It exposes a **retail API with a totally different auth model** than PM2b was built on: **Ed25519 API keys** (`key_id` UUID + `secret_key` base64) via the official **`polymarket-us` SDK** against **`https://api.polymarket.us`** — **not** EIP-712 / `py-clob-client` / `clob.polymarket.com`. So the PM2b "$0 empty twin" wasn't a wrong login — the whole `py-clob-client` path **cannot reach the regulated account** and must be rebuilt. **`.env` restaged** (removed dead `POLYMARKET_PRIVATE_KEY/_FUNDER_ADDRESS/_SIGNATURE_TYPE`; added `POLYMARKET_KEY_ID` + `POLYMARKET_SECRET_KEY`). **Operator next step:** generate keys at `https://polymarket.us/developer` (iOS app + KYC, already verified) → paste into `.env`. **Then (code rework, scope-first):** rewrite `PolymarketClient` on `polymarket-us` keeping the `MarketClient` contract; rework `app/config.py` polymarket creds; revisit `market_registry` (US uses `marketSlug`, not CLOB `token_id`); update PM2b tests. **Full walkthrough + rework checklist: [`docs/setup/polymarket-us-setup.md`](setup/polymarket-us-setup.md).** ~~*(Superseded diagnosis:* the PM2b auth smoke test passed mechanically but funder `0x3D295D…6591` showed $0 across CLOB/on-chain/Data-API; theorized as a per-sign-in-method Magic twin account requiring re-login. Wrong — it was the wrong product/API entirely.)~~ NOTE: `.env` has `DRY_RUN=false` (normal for live Kalshi) — fine today since no Polymarket order path is wired, but the rebuilt PM2c goes live the moment it ships. |
