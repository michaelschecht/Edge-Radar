...
| ~~PM1c~~ | **Dry-run evidence persistence (edge-proving infra)** | Medium | Small | **SHIPPED 2026-07-20.** The Phase 1→2 gate is "prove edge in dry-run," but the scanner accepted `--save` and silently discarded it — no evidence could accumulate. Now `--save` appends every run (timestamp, filter, count, each opp **with its preflight gate verdict**; zero-opp runs logged too) to `data/polymarket/dryrun_log.jsonl` and writes the standard markdown report to `reports/Polymarket/` (new `report_writer` type). First live record captured (NBA Spurs +4.0% low-conf, gated on score). +3 tests (557 total). Run `scan.py polymarket --filter futures --save` on a cadence (manual or scheduled) to build the PM2 case. |
