# My Web Dashboard Architecture

**How my Streamlit dashboard connects to Edge-Radar's core scripts.**
...
|-------------|-----|-------------|
| **My Cloud Instance** | [edge-radar.streamlit.app](https://edge-radar.streamlit.app) | Streamlit Cloud secrets |
| **Local** | `http://localhost:8501` | `.env` + optional `secrets.toml` |

Both environments run the same code. See [Setup Guide](SETUP.md) for my Cloud configuration.
