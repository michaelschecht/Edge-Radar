...
The scan table shows: **Sport** (NBA/NHL/MLB/etc.), **Bet** (matchup), **Type** (ML/Spread/Total/Prop), **Pick** (e.g., "Spurs win", "Over 220.5", "Blazers -7.5"), **When**, **Mkt**, **Fair**, **Edge**, **Conf**, **Score**, **Gate** (R18 — `ok` or a short label like `off` / `illiq` / `far` / `score` / `conf` / `no-fav` / `pred-off` / `live-off` showing which risk gate would reject this row at execute time).
...
- **Sizing is whole-account; spending is per-shard (X1, 2026-08-27):** Kalshi **sharded the exchange on 2026-08-24** — Crypto to shard 2, **Tennis & Baseball (i.e. all of MLB) to shard 3**, everything else on shard 0 — and **cash does not follow the markets**. `bankroll` is `get_balance()["balance"]`, the **sum across every shard** (operator's call 2026-08-27, verified: `8806` before and after a $15 inter-shard move), so an order can be correctly sized against the full balance and still be unspendable where it lands. The venue's error for that is `404 user_not_found` — the market resolves, then the per-shard user lookup does not. `shard_funding.ensure_shard_funded()` runs immediately before each order and moves **exactly the shortfall** from `SHARD_FUNDING_SOURCE` (never a round-up — cash parked on the sports shard cannot back an NFL order), refusing above `MAX_AUTO_SHARD_TRANSFER` (live $25). Kalshi's transfer is **explicitly non-atomic**, so the destination balance is re-read afterwards and the order skipped if still short. Fails **open** on an unknown shard, **closed** on a transfer that did not settle. `AUTO_SHARD_TRANSFER=false` ⇒ the order is **skipped and logged `shard_underfunded`**, never placed to fail. **`cancel_order()` needs `exchange_index` for an off-shard order** — without it the cancel returns a bare `404 not_found`, which the R4 janitor cannot distinguish from "already gone". `doctor.py` prints the per-shard split under the balance.

- **A sport with no settled history does not get live money (S1, 2026-08-26):** NFL is switched off in the live `.env` (`MIN_EDGE_THRESHOLD_NFL=1.0`, the F3 unreachable-floor idiom) — 24 open live positions, **$28.50 = 31% of bankroll**, entries back to 2026-05-23, and **zero** NFL rows in the settlement log; its `margin_stdev: 13.5` is a hardcoded prior, not a fit. **The `.env` key is a tourniquet, not the rule** — the rule is that a cold-start segment runs in dry-run/pilot until it has evidence, and it becomes mechanical when `strategy_state.json` (S10) ships, at which point the key comes out. **The 24 existing positions are held, not flattened** (S2): exiting a 5-20¢-wide book pays exactly the illiquidity penalty Gate 3.6 exists to avoid. A pre-declared review is armed for **Tuesday 2026-09-15** (Windows task `NFL-Week1-Review`), owned by `scripts/backtest/nfl_week1_review.py` — the only thing permitted to change that key.
- **Liquidity floor (L2, 2026-08-18):** Gate 3.6 rejects markets whose bid/ask spread exceeds `MAX_BID_ASK_SPREAD` (default 0.05 — dollars on a $0-1 contract) or whose trailing-24h volume is below `MIN_MARKET_VOLUME_24H` (default 0 = off). This enforces the CLAUDE.md Hard Stop "the market is clearly illiquid (spread > 5%)", which had been documented since launch but **never implemented** — spread reached scoring only through the soft 20%-weight `liquidity` composite term, which saturates at 0 only at a 50¢ spread, so a 20¢-wide book still scored 6.0/10 there and cleared `MIN_COMPOSITE_SCORE`. The 2026-08-18 NFL Week 1 audit found 13 of 27 open positions past the documented line and 18 of 27 with zero 24h volume. Two consequences drove the fix: edge measured against an ask nobody trades at is largely the spread itself, and a zero-volume book offers no exit before settlement. **Fails open** — an opportunity carrying no recorded spread is not rejected. Preflight label: `illiq`. Either knob at 0 disables that half.
