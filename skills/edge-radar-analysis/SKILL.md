...
- **`scripts/backtest/correlation_check.py`** (C11b, 2026-07-27) — measures whether bets in the same cluster `(series, market type, date, side)` resolve together, i.e. whether a slate needs sizing damped beyond the `batch_size` divisor. **Read the stratified figure, never the pooled one**: clusters sit inside strata with very different base rates (totals ~82% WR, spreads ~24%), so pooling manufactures apparent correlation. The 2026-07-27 reading was pooled ρ +0.181 (p=0.0018) vs stratified ρ +0.048, and totals specifically ρ −0.187 (p=0.75) — which is why no correlation guard exists. Re-run when the clustered-bet count roughly doubles; 28 clustered totals bets cannot detect a small ρ.
