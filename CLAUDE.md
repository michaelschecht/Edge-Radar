...
> **The live `.env` deliberately overrides several of these.** Current operator settings (bankroll ≈ $89, so the shipped defaults are sized for a much larger account): `UNIT_SIZE=.50`, `MAX_BET_SIZE=15`, `MAX_DAILY_LOSS=30`, `MAX_BET_RATIO=5`, `MIN_EDGE_THRESHOLD_MLB=0.03`, plus the two longshot-oriented changes below. Run `python scripts/doctor.py` to see what is actually in force — the values in this block are the code defaults, not the running config.
>
> - **`KELLY_FRACTION=1` (2026-07-22, was 0.25).** Reads as "full Kelly" but isn't: `kalshi_executor.py:785` divides it by `batch_size = min(len(opportunities), --max-bets)`, so at the schedulers' `--max-bets 5` the effective multiplier is **0.20 Kelly** (previously 0.05). ~4x on Kelly-driven sizing; the practical effect is that low-priced legs now size off Kelly instead of falling back to the `UNIT_SIZE` flat floor.
> - **`MIN_MARKET_PRICE=0.10` (2026-07-22, was 0.12).** Intentional re-opening of the longshot lane. See the R7 note below for the original rationale and the `.env` comment for the current full-history read (sub-15c: 6W-47L / 53 bets; headline +47.5% ROI is 99% a single trade). **Open experiment — recheck after ~30 more settles.**
