...
> **🔴 NEXT UP — Polymarket integration (highest priority).** Account funded 2026-07-14 — it's the **CFTC-regulated Polymarket US** product (iOS-app only), which uses an **Ed25519 retail API** (`api.polymarket.us`), *not* the international EIP-712/`py-clob-client` scheme. As of 2026-07-20 the execution client is **rebuilt** on that API and the **futures scanner reads US market data** (edge priced on US quotes, real `marketSlug` recorded) — both live-verified — behind the existing provider-agnostic `Opportunity` + `size_order` risk-gate chain. Phased: **Phase 1 read-only/dry-run → prove edge → Phase 2 execution.** Remaining: execution-pipeline wiring, then a seasonal games repoint (US game markets are moneyline-only; no spreads/totals/MLB). Full detail: **[docs/setup/polymarket-us-setup.md](docs/setup/polymarket-us-setup.md)** + **[docs/ROADMAP.md](docs/ROADMAP.md) Priority 0 (PM2c-0)**.
