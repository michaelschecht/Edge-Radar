...
| **Web Dashboard** | Streamlit app — scan, execute, portfolio, settle, config; **both venues** (Kalshi + Polymarket) | Deploy your own (see `docs/web-app/LOCAL.md`) |
...
>
> **Update 2026-07-23 (C10):** the dry-run gate could not terminate — the futures composite scaled edge 5x more strictly than sports, making Gate 4 unreachable for any realistic futures edge (and explaining **0 futures bets in 85 settled trades** on Kalshi too). Fixed; the gate is now reachable, though nothing observed newly qualifies. **C10b (2026-07-31)** applied the same fix to the Polymarket *games* path, which C10 had missed: 362 logged game rows, none ever above composite 5.30 against a 6.0 gate. See the C10 / C10b notes below.
...
│   └── views/                       # Page modules (scan, portfolio, settle, backtest, config)
├── tests/                           # 677 pytest tests
└── scripts/
...
**C10b (2026-07-31): the same fix applied to Polymarket *games*.** C10 aligned both futures paths but missed `scripts/polymarket/polymarket_games_edge.py`, which was written on 07-20 — three days earlier — and had copied `edge * 20` from `polymarket_futures_edge.py`, which had itself copied it from the `liquidity` line above it. A copy of a copy of the same bug, so it had no independent rationale either. Same consequence, independently confirmed on this surface: clearing `MIN_COMPOSITE_SCORE=6.0` needed **~15% edge at high confidence / 26% medium / 38% low** against game edges that run **1–7%**, and across **362 logged Gamma game rows not one ever reached 6.0** (max observed **5.30**) — Gate 4 was structurally unreachable here too. **Not a floodgate:** replayed through the shipped code over those same 362 rows, only **5 (1.4%)** newly clear Gate 4, all marginally (6.02–6.26), and each still faces gates 3.5 / 4.5 / 4.6b / 5 / 6 / 7; the other **330** are stopped at Gate 3 (edge) and never reach Gate 4 at all. Games are **not executable today** regardless (Gamma rows carry no US `market_slug` and are auto-excluded), so this changes no live behavior — it matters for the seasonal US games repoint, so that surface doesn't inherit the same unreachable gate a third time. Two divergences from the sports composite are **deliberately kept**: the liquidity term stays `book_spread * 100` (rows wider than `MAX_BOOK_SPREAD=0.10` are already dropped, so `* 20` would compress every survivor into 9.8–10.0 and carry no information), and `high: 9` stays uncapped on C10's own precedent — C4's evidence is 306 settled *Kalshi* bets, and there is still no settled Polymarket data. Revisit the confidence weight when PM3 settlement lands; this path prices off the same Odds API consensus as sports, so C4's *reasoning* plausibly transfers even though its evidence doesn't. No env var.
